...
**********************************************************/
#define VERSION "0.0.3 2004-11-09"
#include <iostream>
#include <fstream>
#include <map>
#include <stdlib.h>
...
static char name[5] = "CSST" ;	/* Default client name			*/
char sname[6] = "*" ;		/* Default station list.		*/
...
/* Sending an attach command is harmless */
	  ...
	  if (report_interval != 0 && (now - last_report) > report_interval ) {
		output_statistics("Interval Reporting of Statistics:");
      		last_report = now;
	  }
          j = cs_scan (me, &alert) ;
          if (j != NOCLIENT)
			...
			fflush (info);
			  ...
			  {
				char * cl_str;
				// we have a new location code and channel
		...
		output_statistics("SIGHUP SIGNAL Reporting of Statistics:");
		flush_statistics = 0;
    	    }
        }
...
//
//  Scan the types field in the command line
// 
