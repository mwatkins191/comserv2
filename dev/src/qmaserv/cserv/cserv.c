...
/*PJM - QMA change to include a def to comment out lines easily */
#define QMA 1 /* Define QMA to comment out stuff easily */
...
/*PJM - QMA change for debugging. Set debug to high if needed */
boolean rambling = FALSE ; /* Incoming packets display a line */
boolean insane = FALSE ;   /* Client commands are shown */
/* boolean rambling = TRUE ;*/ /* Incoming packets display a line */
/* boolean insane = TRUE ; */  /* Client commands are shown */
...
/*
  QMA - This needs access to socket to Q330 to send a disconnect message.
  Defer that until later. At this point, comserv exits, and doesn't
  disconnect properly.
*/
...
/*PJM - QMA change. Moved these externally so init, and scan routines can
  be called as seperate functions.
*/
...
/*PJM - QMA change making the init routine a subroutine */

int comserv_init (char* station_code)
    {
#ifdef _OSK
      ...
      int shmid ;
      int semid;
      short j, k, l, zerocnt, first;
      long total, lasttotal, cflag ;
      short min, max ;
      int err, outque ;
      char c ;
      long ctmax, ctmin;
...
/*PJM - QMA change the input to a single string parameter - the station
  code. Modify the printed information to ID this as a mountainair server.
*/
      printf ("%s qmaserv Edition %d started for station %s\n", 
...
/* Check for that this is a comlink station */
      if (strcasecmp((pchar) &source, "comlink") != 0)
          terminate ("Not a comlink configuration.\n") ;
...
/*PJM - QMA label used to comment out a sections
...
/*PJM - QMA End of commented out port configu stuff */
#endif 

/*PJM - QMA. Signal handling will be done in main loop, so these handlers
   are commented out. Equivalent code will be written in the main loop.
*/
...
/* PJM - QMA comment out the link status monitoring initial request
   link call.
*/
#ifndef QMA
      if (linkstat.ultraon)
...
/* PJM - QMA end of iniitliazation subroutine. Always exit, or return 1 here.*/
 return 1;
 } /* End of init routine */


/* PJM - QMA Client scan routine checks for service requests from clients */
/* The main QMA routine should poll this routine to ensure clients data */
/* requests are met in a timely manner. */
 int retVal = 0;
 int comserv_scan()
        {
          retVal = 0;
          //printf("--- In comserv_scan checking clients.\n");
          check_input () ;
#ifdef _OSK
          ...
          if ((curtime - lastsec) >= 1.0)
              {
                ...
                check_clients () ;
/* 
  PJM - QMA. This is link monitoring code. Comment it out because link
  monitoring is done in the qma main loop.
*/
#ifndef QMA
                if (! serial)
                    if ((path >= 0) && ! udplink)
          ...
          if (verbose && (noackmask != oldackmask))
              {
                for (i = DATAQ ; i <= STOPACK ; i++)
                  if (verbose && (test_bit(noackmask eor oldackmask, i)))
                      {
...
/* PJM - QMA end of commented out link status monitoring section */
#endif
              }
#ifdef SOLARIS2
          nanosleep (&rqtp, &rmtp) ;
...
/*PJM - QMA change to check this each loop, but no
  execute it unless stop has been set. 
  Previously the code was done as a while loop.
  Now it's a function call.
*/
   if (stop)
         {
      if (serial)
