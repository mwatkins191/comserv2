#ifndef lint
static char sccsid[] = "@(#) $Id: datalog_utils.c,v 1.1.1.1 2004/06/15 19:08:00 isti Exp $";
#endif
...
FINFO *find_finfo (DATA_HDR *hdr) 
{
	...
	fprintf (info, "Unknown blockette/packet type %d for %\n", n,
	...
	if ((fip = (FINFO *)malloc(sizeof(FINFO))) == NULL) {
	    fprintf (info, "Unable to malloc FINFO for %\n",
		     channelstring(hdr->station_id, hdr->location_id, hdr->channel_id, hdr->network_id));
	    terminate_program (1);
	}
